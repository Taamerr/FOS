...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	if(found != E_SEMAPHORE_NOT_EXISTS){
		...
		int sucAlloc = allocate_semaphore_object(&allocatedObject);
		if(sucAlloc != E_NO_SEMAPHORE){
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	struct Semaphore *temp = semaphores + (found * sizeof(struct Semaphore));
	semaphores[found].value -= 1;
