...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	//	c) E_NO_SHARE if the number of shared objects reaches max "MAX_SHARES"
 	int found = get_share_object_ID(ownerID, shareName);
	struct Share *allocatedObject = NULL;
	if(found != E_SHARED_MEM_NOT_EXISTS){
		return E_SHARED_MEM_EXISTS;
	}
	else{
		int sucAlloc = allocate_share_object(&allocatedObject);
		if(sucAlloc != E_NO_SHARE){
			allocate_chunk(curenv->env_page_directory,(uint32)virtual_address,size, PERM_WRITEABLE | PERM_USER);
			...
			uint32 aaa = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE;
			uint32 startVa = (uint32)virtual_address;
			for(int i=0; i < aaa; i++){
				uint32* ptr_page_table = NULL;
				struct FrameInfo* ptr_frame_info = NULL;
				get_page_table(curenv->env_page_directory, startVa, &ptr_page_table);
				ptr_frame_info = get_frame_info(curenv->env_page_directory, startVa, &ptr_page_table);
				pt_set_page_permissions(curenv->env_page_directory, startVa, PERM_WRITEABLE, 0);
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	struct Share temp = shares[found];
	uint32 numOfPages = ROUNDUP(temp.size,PAGE_SIZE) / PAGE_SIZE;
	uint32 va = (uint32) virtual_address;
	for(int j=0;j<numOfPages;j++){
		uint32* ptr_table = NULL;
		get_page_table(curenv->env_page_directory, va, &ptr_table);
		struct FrameInfo *ptr_frame_info = NULL;
		ptr_frame_info = get_frame_from_storage(temp.framesStorage, j);
		if(temp.isWritable == 0)
			map_frame(curenv->env_page_directory, ptr_frame_info, va, PERM_USER);
		else{
			map_frame(curenv->env_page_directory, ptr_frame_info, va, PERM_WRITEABLE | PERM_USER);
		}
		va += PAGE_SIZE;
	}
	if(found == E_SHARED_MEM_NOT_EXISTS){
