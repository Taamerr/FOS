...
void initialize_dyn_block_system()
{
	...
	sys_allocate_chunk(USER_DYN_BLKS_ARRAY, MAX_MEM_BLOCK_CNT*(sizeof(struct MemBlock)), PERM_WRITEABLE | PERM_USER);
...
void free(void* virtual_address)
{
	...
	sys_free_user_mem(va,blockTodelete->size);
...
void* smalloc(char *sharedVarName, uint32 size, uint8 isWritable)
{
	...
	if(insertedBlock != NULL)
	{
		int res = sys_createSharedObject(sharedVarName, size, isWritable,(uint32*)insertedBlock->sva);
...
void* sget(int32 ownerEnvID, char *sharedVarName)
{
	...
	uint32 s = ROUNDUP(sizeOfSharedObject, PAGE_SIZE);
	if(sizeOfSharedObject == E_SHARED_MEM_NOT_EXISTS){
		return NULL;
	}
	struct MemBlock * insertedBlock;
	if(sys_isUHeapPlacementStrategyFIRSTFIT() == 1){
		insertedBlock = alloc_block_FF(s);
	}
	if(insertedBlock != NULL)
	{
		int res = sys_getSharedObject(ownerEnvID ,sharedVarName,(uint32*)insertedBlock->sva);
		if(res == E_SHARED_MEM_EXISTS){
			return NULL;
		}
		insert_sorted_allocList(insertedBlock);
		return (void*) insertedBlock->sva;
